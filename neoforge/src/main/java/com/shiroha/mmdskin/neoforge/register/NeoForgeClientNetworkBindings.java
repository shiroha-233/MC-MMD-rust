...
final class NeoForgeClientNetworkBindings {
    ...
    void register() {
        ...
        ActionWheelNetworkHandler.getInstance().setNetworkSender(animId -> {
            ...
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withAnimId(NetworkOpCode.CUSTOM_ANIM, player.getUUID(), animId));
            }
        });
        ...
        ActionWheelNetworkHandler.getInstance().setAnimStopSender(() -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withArg(NetworkOpCode.RESET_PHYSICS, player.getUUID(), 0));
            }
        });
        ...
        PlayerMorphSyncService.getInstance().setMorphSender(morphName -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withAnimId(NetworkOpCode.MORPH_SYNC, player.getUUID(), morphName));
            }
        });
        ...
        ModelSelectorNetworkHandler.getInstance().setNetworkSender(modelName -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withAnimId(NetworkOpCode.MODEL_SELECT, player.getUUID(), modelName));
            }
        });

        PlayerModelSyncManager.setNetworkBroadcaster((playerUUID, modelName) ->
            ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withAnimId(NetworkOpCode.MODEL_SELECT, playerUUID, modelName)));

        MaidModelNetworkHandler.getInstance().setNetworkSender((entityId, modelName) -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.forMaid(NetworkOpCode.MAID_MODEL, player.getUUID(), entityId, modelName));
            }
        });
        ...
        MaidActionNetworkHandler.getInstance().setNetworkSender((entityId, animId) -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.forMaid(NetworkOpCode.MAID_ACTION, player.getUUID(), entityId, animId));
            }
        });
        ...
        StageNetworkHandler.setStageMultiSender(payload -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withAnimId(NetworkOpCode.STAGE_MULTI, player.getUUID(), payload));
            }
        });
        ...
        BoneSyncNetworkHandler.setNetworkSender(payload -> {
            LocalPlayer player = minecraft.player;
            if (player != null) {
                ClientPacketDistributor.sendToServer(MmdSkinNetworkPack.withBinary(NetworkOpCode.BONE_SYNC, player.getUUID(), payload));
            }
        });
    }
}
