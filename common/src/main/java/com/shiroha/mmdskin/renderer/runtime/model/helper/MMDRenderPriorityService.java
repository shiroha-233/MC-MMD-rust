 ...
 * MMD 世界场景优先级与配额服务。
 */
public final class MMDRenderPriorityService {
    private static final MMDRenderPriorityService INSTANCE = new MMDRenderPriorityService();
    ...
    private MMDRenderPriorityService() {
    }

    public static MMDRenderPriorityService get() {
        return INSTANCE;
    }
    ...
    public synchronized void beginWorldFrame() {
        ...
        if (currentFrameKey != Long.MIN_VALUE) {
            MMDPerformanceProfiler.get().completeFrame(visibleModelsThisFrame, physicsModelsThisFrame);
        }
    ...
    public synchronized boolean shouldUseMmdPlayer(AbstractClientPlayer player) {
        beginWorldFrame();
        if (player == null) {
            return false;
        }
